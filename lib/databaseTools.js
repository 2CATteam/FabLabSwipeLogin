...
class DBTools {
    ...
    createUser(id, name, email, login, strings) {
        return new Promise((res, rej) => {
            ...
            this.db.all("SELECT * FROM guests WHERE guest_id = ?", id, async (err, rows) => {
                ...
                //Try again once...
                if (!canvasId) {
                    canvasId = await canvas.enrollUser(id, email, name, strings.canvasWelcomeTitle, strings.canvasWelcomeBody)
                }
                            ...
                            if (row.canvas_id) {
                                ...
                                for (let i in certs) {
                                    if (certs[i].id == cert && certs[i].assignmentId) {
                                        ...
                                        canvas.removeCert(row.canvas_id, certs[i].assignmentId, reason).catch((err) => {
                                            console.error("Was unable to revoke cert on canvas for user", user, ". Their ID should be purged from the DB")
                                        })
                                        break
                                    }
                                }
    ...
    deleteTask(id) {
        return new Promise((res, rej) => {
            this.db.run("DELETE FROM tasks WHERE task_id = ?", id, (err) => {
                if (err) rej(err)
                res()
            })
        })
    ...
    messageUser(guest_id, title, body) {
        this.getUser(guest_id).then(async (data) => {
            if (data.canvas_id) {
                canvas.messageUser(data.canvas_id, title, body).catch((error) => {
                    console.error("Failed to send Canvas DM to user", guest_id, ", emailing instead")
