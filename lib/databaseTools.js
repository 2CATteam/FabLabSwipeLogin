...
class DBTools {
            ...
            this.db.get("SELECT * FROM current WHERE guest_id = ?;", id, (err, row) => {
                ...
                if (row) {
                    this.db.run("DELETE FROM current WHERE guest_id = ?;", id, (err) => {
                        if (err) rej(err)
                        this.db.run("UPDATE history SET resolved = 1, note = ? WHERE guest_id = ? AND resolved = 0 AND type = ?", new Date().toISOString(), id, history_types.VISIT, (err) => {
                            if (err) rej(err)
                            res(false)
                        })
                    })
    ...
    async createTask(name, description, period, date) {
        return new Promise((res, rej) => {
