...
Once you've taken care of those two things, you're good to go! Let us know if you have any questions or problems, either by talking to a staff member in the shop, or emailing us at fablab@ou.edu`)
    ...
    getCache() {
        return new Promise((res, rej) => {
            this.db.all("SELECT guests.guest_id, guests.name, guests.email, guests.certs FROM history, guests WHERE guests.guest_id = history.guest_id AND history.type = ? AND history.date BETWEEN strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-1 day') AND strftime('%Y-%m-%dT%H:%M:%fZ', 'now') GROUP BY guests.guest_id", history_types.VISIT, (err, rows) => {
                if (err) rej(err)
                res(rows)
            })
        })
