...
Once you've taken care of those two things, you're good to go! Let us know if you have any questions or problems, either by talking to a staff member in the shop, or emailing us at fablab@ou.edu`)
    ...
    checkExpirationTimer(certs, warningThreshold, removalThreshold, checkPeriod) {
        return new Promise(async (res, rej) => {
            ...
            this.db.each("SELECT * FROM guests", (err, row) => {
                ...
                this.db.all("SELECT * FROM history WHERE guest_id = ? AND (type = ? OR type = ? OR type = ?) ORDER BY date DESC",
                    row.guest_id, history_types.VISIT, history_types.ADD_CERT, history_types.AUTO_ADD, (err, rows) => {
                        ...
                        if (row.canvas_id) {
                            ...
                            this.doExpiration(row.guest_id, row.certs, certTimes, certs, warningThreshold, removalThreshold, checkPeriod)
                        }
                        doneCount++
                    }
                )
                ...
                for (let i in pending) {
                    ...
                    if (chunk.length >= 10) {
                        ...
                        for (let j in chunk) {
                            this.doExpiration(pending[chunk[j]].guest_id, pending[chunk[j]].certs, pending[chunk[j]].times, certs, warningThreshold, removalThreshold, checkPeriod)
                        }
                        chunk = []
                    }
                }
    ...
    async doExpiration(guestId, userCertifications, certTimes, certs, warningThreshold, removalThreshold, checkPeriod) {
        ...
        if (warnings.length > 0) {
            ...
            this.messageUser(guestId, "Fab Lab tool certifications nearing expiration", `Hey there! Some of your Fab Lab tool certifications are close to expiring. In 14 days, you'll lose the following certifications:
${warningCerts}
...
Once they expire, don't worry! You'll just need to retake the certification quizzes before you use their respective tools.`)
        ...
        if (expired.length > 0) {
            ...
            this.messageUser(guestId, "Fab Lab tool certifications have expired", `Hey there! In an effort to make sure our shop is as safe as possible, our tool certifications regularly expire, after 45 days of not being used.
